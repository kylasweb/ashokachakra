...
class TestSecurityChecks(unittest.TestCase):
    ...
    def test_PE(self):
        ...
        self.assertEqual(
            ...
            (1, f'{executable}: failed DYNAMIC_BASE HIGH_ENTROPY_VA NX'),
        )
        self.assertEqual(
            call_security_check(
                cc,
            ...
            (1, f'{executable}: failed DYNAMIC_BASE HIGH_ENTROPY_VA'),
        )
        self.assertEqual(
            call_security_check(
                cc,
    ...
    def test_MACHO(self):
        ...
        self.assertEqual(
            call_security_check(
                ...
                source,
                executable,
                ['-Wl,-no_pie', '-Wl,-flat_namespace', '-Wl,-allow_stack_execute'],
            ),
            (1, f'{executable}: failed PIE NOUNDEFS NX'),
        )
        self.assertEqual(
            call_security_check(
                cc, source, executable, ['-Wl,-no_pie', '-Wl,-flat_namespace']
            ),
            (1, f'{executable}: failed PIE NOUNDEFS'),
        )
        self.assertEqual(
